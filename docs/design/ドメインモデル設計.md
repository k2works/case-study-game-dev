...
```plantuml
@startuml "AIドメインサービス"
class AIEvaluationService <<DomainService>> {
  +evaluatePosition(field: Field, puyo: PuyoPair, strategy: StrategyValueObject): number
  +findOptimalMove(field: Field, puyo: PuyoPair, strategy: StrategyValueObject): AIDecision
  +predictChainPotential(field: Field, move: Move): ChainPrediction
  +calculateRiskLevel(field: Field): RiskLevel
}
...
AIEvaluationService --> StrategyValueObject
