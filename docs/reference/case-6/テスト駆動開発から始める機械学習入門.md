...
本記事は、テスト駆動開発（TDD）を実践しながら Python で機械学習を学ぶプロジェクトの完全ガイドです。４章から８章までの6つの段階を通じて、データ処理の基礎から実用的な機械学習 API まで、段階的にスキルアップできる構成になっています。
...
機械学習開発では、**Jupyter Lab** が非常に重要なツールです。「コードを書いて、すぐに結果を確認する」というサイクルを高速で回せるため、データ分析や機械学習の実験に最適です！

#### Jupyter Lab とは？

**Jupyter Lab** は、インタラクティブな開発環境です。ブラウザ上で Python コードを実行し、その結果を即座に確認できます。
...
# セル 2（セル1の結果を保持したまま実行）
df.head()  # すぐに結果が表示される

# セル 3（必要な部分だけ再実行）
df.describe()  # 他のセルを実行せずに確認できる
```

**Jupyter Lab の強み**：
- 📊 **データの可視化が簡単**: グラフが即座に表示される
- 🔍 **探索的分析に最適**: 少しずつコードを試しながら分析できる
- 📝 **ドキュメントとコードの統合**: マークダウンでメモを書きながら開発
- 🎯 **試行錯誤が楽**: セル単位で実行・修正できる

#### 🛠️ Jupyter Lab のインストール

uv を使えば、簡単にインストールできます：
...
# 特定のディレクトリで起動
uv run jupyter lab --notebook-dir=./notebook
```

起動すると、インターフェースが表示されます：
...
**1. Variable Inspector（変数ビューア）**

現在のメモリ上の変数を一覧表示：

```bash
uv add --dev jupyter-lsp jupyterlab-lsp
```

**2. Code Formatter（コードフォーマッター）**
...
設定後、右クリック → Format Cell で整形できます。

**3. Table of Contents（目次）**

マークダウン見出しから自動で目次を生成：

Jupyter Lab 標準機能として含まれています（左サイドバーの目次アイコン）。
...
print(f"R²: {score}")  # 結果を確認

# ↓ 動作確認できたら本番コードへ移行

# 📝 本番コード（src/ml/cinema_predictor.py）
...
# ✅ テストコード（test/test_cinema_predictor.py）
def test_model_training():
    predictor = CinemaPredictor()
    X = pd.DataFrame({'SNS1': [100, 200], 'SNS2': [300, 400]})
    y = pd.Series([1000, 2000])

    score = predictor.train(X, y)
    assert score > 0.9  # TDD でしっかり検証
```

**この組み合わせの利点**：

- 🔍 **Jupyter Lab**: 高速な試行錯誤、データの可視化
- ✅ **TDD**: 品質保証、リファクタリングの安全性
- 📦 **本番コード**: モジュール化、再利用性

#### 📊 各章での Jupyter Notebook の活用

各章では、以下のような Jupyter Notebook を作成します：

- **4章（Iris）**: `notebook/iris_tutorial.ipynb`
    - データ探索
    - 分類モデルの訓練
    - 結果の可視化

- **5章（Cinema）**: `notebook/cinema_tutorial.ipynb`
    - 回帰問題の理解
    - 特徴量の分析
    - 予測結果の評価
...
Jupyter Notebook はインタラクティブな学習ツールとして、本書の各章で積極的に活用していきます！
...
```bash
uv run python script/train_iris.py
...
モデルの訓練が完了したら、Jupyter Notebook を使って結果を詳しく分析しましょう！対話的な環境で、データやモデルの挙動を深く理解できます。

#### 🎯 この節の目的

- **データの分布を視覚的に理解する** - 特徴量間の関係性を把握
- **モデルの性能を多角的に評価する** - 混同行列、決定木の可視化
- **実験を素早く繰り返す** - パラメータを変えて結果を比較

#### 📝 Notebook の作成

プロジェクトルートで Jupyter Lab を起動します：
...
`notebook/` ディレクトリに新しいノートブック `iris_exploration.ipynb` を作成しましょう。
...
print(f"クラス: {y.unique()}")
```

#### 2️⃣ データ分布の視覚化

```python
# セル 3: 特徴量の分布をペアプロットで確認
# データフレームにラベルを追加
df_viz = X.copy()
df_viz['species'] = y

# ペアプロット（時間がかかる場合があります）
...
pairplot.fig.suptitle('Iris データセットの特徴量分布', y=1.02, fontsize=16)
plt.tight_layout()
plt.show()

print("✅ ペアプロット完成！各特徴量の関係性が見えますね")
```

**このグラフから分かること**：
- `petal_length` と `petal_width` で種類がよく分離できる
- `setosa` は他の2種と明確に区別できる
- `versicolor` と `virginica` は一部重なる
...
for idx, feature in enumerate(features):
    ax = axes[idx // 2, idx % 2]
    df_viz.boxplot(column=feature, by='species', ax=ax)
    ax.set_title(f'{feature} の分布')
    ax.set_xlabel('種類')
    ax.set_ylabel(f'{feature} (cm)')

plt.suptitle('特徴量別の箱ひげ図', fontsize=16)
plt.tight_layout()
plt.show()
```
...
**混同行列の読み方**：
- 対角線上の値が大きい = 正しく分類できている
- 対角線外の値 = 誤分類（どのクラスを間違えやすいか分かる）
...
print("✅ 決定木の可視化完了！どの特徴量で分岐しているか見てみましょう")
```

**決定木から学べること**：
- 最初にどの特徴量で分岐するか（= 最も重要な特徴量）
- 各ノードでどのように分類が進むか
- 葉ノードの色の濃さ = その予測の確信度
...
```python
# セル 9: 異なる max_depth での性能比較
depths = range(1, 10)
train_scores = []
test_scores = []

for depth in depths:
    clf = IrisClassifier(max_depth=depth)
...
# 結果の可視化
plt.figure(figsize=(10, 6))
plt.plot(depths, train_scores, marker='o', label='訓練データ')
plt.plot(depths, test_scores, marker='s', label='テストデータ')
plt.xlabel('決定木の深さ (max_depth)')
plt.ylabel('正解率')
plt.title('max_depth による性能の変化')
plt.legend()
...
print(f"\n最適な max_depth: {optimal_depth}")
print(f"最高テスト精度: {max(test_scores):.4f}")
```

**このグラフから分かること**：
- `max_depth` が小さすぎる = 学習不足（underfitting）
- `max_depth` が大きすぎる = 過学習（overfitting）
- テストデータの精度が最大になる点が最適
...
| ✅ やること | ❌ 避けること |
|------------|-------------|
| データの分布を視覚的に確認 | 最初から完璧なコードを書こうとする |
| パラメータを変えて実験 | すべてを Notebook で完結させる |
| 気づきをマークダウンでメモ | グラフの意味を考えずに次へ進む |
| 重要な発見を本番コードに反映 | Notebook を散らかしたままにする |

#### 🔄 探索から本番コードへ

Notebook での発見を本番コードに反映する流れ：

1. **Notebook で探索** → 最適な `max_depth` は 3 だと判明
2. **テストに反映** → デフォルト値を検証するテストを追加
3. **本番コードに反映** → `IrisClassifier` のデフォルトを更新
4. **ドキュメント化** → README に最適パラメータを記載

```python
# 発見: max_depth=3 が最適
# ↓
# test/test_iris_classifier.py に追加
def test_最適なデフォルトパラメータ(self):
    """最適な max_depth がデフォルト値として設定されている"""
    classifier = IrisClassifier()
    assert classifier.max_depth == 3  # Notebook で発見した最適値
```
...
```bash
uv run python script/train_cinema.py
...
回帰モデルでは、予測値と実測値の関係を視覚化することが重要です。Jupyter Notebook で詳しく分析しましょう！
...
`notebook/` ディレクトリに `cinema_exploration.ipynb` を作成しましょう。
...
plt.title('特徴量と興行収入の相関マトリックス', fontsize=14)
plt.tight_layout()
plt.show()
...
**相関から分かること**：
- どの特徴量が興行収入と強く関連しているか
- 特徴量間に多重共線性がないか

```python
# セル 4: 各特徴量と興行収入の関係を散布図で確認
fig, axes = plt.subplots(2, 2, figsize=(14, 10))
features = X.columns

for idx, feature in enumerate(features):
    ax = axes[idx // 2, idx % 2]
    ax.scatter(X[feature], y, alpha=0.5, s=50)
    ax.set_xlabel(feature, fontsize=12)
    ax.set_ylabel('興行収入 (万円)', fontsize=12)
    ax.set_title(f'{feature} と興行収入の関係', fontsize=13)
...
# 評価指標
metrics_train = predictor.evaluate(y_train, y_pred_train)
metrics_test = predictor.evaluate(y_test, y_pred_test)

print("【訓練データの評価】")
print(f"  R²   : {metrics_train['r2']:.4f}")
print(f"  MAE  : {metrics_train['mae']:.2f} 万円")
print(f"  RMSE : {metrics_train['rmse']:.2f} 万円")

print("\n【テストデータの評価】")
print(f"  R²   : {metrics_test['r2']:.4f}")
print(f"  MAE  : {metrics_test['mae']:.2f} 万円")
print(f"  RMSE : {metrics_test['rmse']:.2f} 万円")
```
...
# 訓練データ
ax1.scatter(y_train, y_pred_train, alpha=0.5, s=50)
ax1.plot([y_train.min(), y_train.max()],
         [y_train.min(), y_train.max()],
         'r--', lw=2, label='理想的な予測')
ax1.set_xlabel('実測値 (万円)', fontsize=12)
ax1.set_ylabel('予測値 (万円)', fontsize=12)
ax1.set_title(f'訓練データ (R²={metrics_train["r2"]:.3f})', fontsize=13)
ax1.legend()
ax1.grid(True, alpha=0.3)

# テストデータ
ax2.scatter(y_test, y_pred_test, alpha=0.5, s=50, color='green')
ax2.plot([y_test.min(), y_test.max()],
         [y_test.min(), y_test.max()],
         'r--', lw=2, label='理想的な予測')
ax2.set_xlabel('実測値 (万円)', fontsize=12)
ax2.set_ylabel('予測値 (万円)', fontsize=12)
ax2.set_title(f'テストデータ (R²={metrics_test["r2"]:.3f})', fontsize=13)
ax2.legend()
...
axes[0, 0].set_xlabel('予測値 (万円)')
axes[0, 0].set_ylabel('残差 (実測値 - 予測値)')
axes[0, 0].set_title('残差プロット (訓練データ)')
...
axes[0, 1].set_ylabel('残差 (実測値 - 予測値)')
axes[0, 1].set_title('残差プロット (テストデータ)')
...
axes[1, 0].set_title('残差の分布 (訓練データ)')
axes[1, 0].grid(True, alpha=0.3)

# テストデータの残差ヒストグラム
axes[1, 1].hist(residuals_test, bins=20, alpha=0.7, color='green', edgecolor='black')
axes[1, 1].axvline(x=0, color='r', linestyle='--', lw=2)
axes[1, 1].set_xlabel('残差 (万円)')
axes[1, 1].set_ylabel('頻度')
axes[1, 1].set_title('残差の分布 (テストデータ)')
axes[1, 1].grid(True, alpha=0.3)

plt.tight_layout()
plt.show()

print(f"残差の統計 (テストデータ):")
print(f"  平均: {residuals_test.mean():.2f} 万円")
print(f"  標準偏差: {residuals_test.std():.2f} 万円")
print(f"  最大誤差: {residuals_test.abs().max():.2f} 万円")
```
...
plt.title('線形回帰の係数（特徴量の寄与度）', fontsize=14)
plt.xlabel('係数の値', fontsize=12)
...
print("\n回帰式:")
print(f"興行収入 = {intercept:.2f}")
for feature, coef in zip(X.columns, coefficients):
    print(f"  + {coef:.2f} × {feature}")
```

**係数の解釈**：
- 正の係数 = その特徴量が増えると興行収入も増える
- 負の係数 = その特徴量が増えると興行収入が減る
- 係数の絶対値が大きい = 影響力が強い
...
test_df['error'] = np.abs(y_test.values - y_pred_test)
test_df['error_pct'] = (test_df['error'] / test_df['actual']) * 100

# 誤差が大きいトップ5
print("誤差が大きいサンプル トップ5:")
print(test_df.nlargest(5, 'error')[['actual', 'predicted', 'error', 'error_pct']])

# 誤差が小さいトップ5
print("\n誤差が小さいサンプル トップ5:")
print(test_df.nsmallest(5, 'error')[['actual', 'predicted', 'error', 'error_pct']])
```

#### 8️⃣ 感度分析（What-if シナリオ）

```python
# セル 10: 特徴量を変化させたときの予測変化
# 基準となるサンプル
...
base_prediction = predictor.predict(base_sample)[0]

# SNS1 を変化させる
sns1_range = np.linspace(0, 300, 50)
predictions_sns1 = []

for sns1_val in sns1_range:
    sample = base_sample.copy()
    sample['SNS1'] = sns1_val
    pred = predictor.predict(sample)[0]
    predictions_sns1.append(pred)

# 可視化
plt.figure(figsize=(10, 6))
plt.plot(sns1_range, predictions_sns1, marker='o', markersize=3)
plt.axhline(y=base_prediction, color='r', linestyle='--', label=f'基準予測 ({base_prediction:.0f}万円)')
plt.xlabel('SNS1 の値', fontsize=12)
plt.ylabel('予測興行収入 (万円)', fontsize=12)
plt.title('SNS1 の変化が予測に与える影響', fontsize=14)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()

print(f"SNS1 が 100 増えると、興行収入は約 {(predictions_sns1[-1] - predictions_sns1[0]) / 3:.0f} 万円増加")
```

#### 💡 回帰モデルでの Notebook 活用ポイント

| ✅ やること | ❌ 避けること |
|------------|-------------|
| 残差プロットで仮定を確認 | R² だけで性能を判断 |
| 予測誤差が大きいサンプルを分析 | 外れ値を無視する |
| 特徴量の寄与度を理解 | 係数の意味を考えない |
| What-if シナリオで感度分析 | 単純な予測だけで終わる |
...
# 発見 1: SNS2 の相関が最も強い
# ↓ テストに反映
def test_SNS2の係数が最大(self):
    """SNS2 の係数が最も大きいことを確認"""
    coefs = dict(zip(predictor.model.feature_names_in_,
                     predictor.model.coef_))
    assert coefs['SNS2'] == max(coefs.values())

# 発見 2: 残差が正規分布に従う
# ↓ テストに反映
def test_残差の正規性(self):
    """残差がおおよそ正規分布に従うことを確認"""
    from scipy import stats
    _, p_value = stats.normaltest(residuals)
    assert p_value > 0.05  # 有意水準 5% で正規性を棄却しない
```
...
```bash
uv run python script/train_survived.py
...
実践的な分類問題では、データの性質を深く理解することが成功の鍵です。Jupyter Notebook で詳しく分析しましょう！

#### 🎯 この節の目的

- **データの傾向を視覚的に把握する** - 生存率に影響する要因を発見
- **クラス不均衡の影響を理解する** - 偏ったデータでの対策
- **モデルの判断基準を可視化する** - 誤分類のパターンを発見
...
`notebook/` ディレクトリに `survived_exploration.ipynb` を作成しましょう。
...
**クラス不均衡から分かること**：
- 約 62% が死亡、38% が生存
- このままでは「全員死亡」と予測すれば 62% の正解率
- `class_weight='balanced'` で対策が必要
...
ax = axes[0]
bars = ax.bar(['女性 (male=0)', '男性 (male=1)'], gender_survival, color=['#e91e63', '#2196f3'])
...
ax = axes[1]
bars = ax.bar(['1等 (上流)', '2等 (中流)', '3等 (下流)'], pclass_survival,
...
**重要度から学べること**：
- `male`（性別）が最も重要 → 「女性と子供を優先」
- `Fare`（運賃）が2番目 → 裕福な人ほど生存率が高い
- `Age`（年齢）も重要 → 若い人の生存率が高い
...
print(f"誤分類サンプル数: {len(misclassified)} / {len(test_df)} ({len(misclassified)/len(test_df)*100:.1f}%)")
...
print("\n正しく分類されたサンプルの平均:")
print(test_df[test_df['correct']][['Pclass', 'Age', 'SibSp', 'Parch', 'Fare', 'male']].mean())
```
...
#### 💡 実践的分類問題での Notebook 活用ポイント

| ✅ やること | ❌ 避けること |
|------------|-------------|
| クラス不均衡を必ず確認 | 正解率だけで判断 |
| 混同行列で誤分類パターンを分析 | AUC を無視する |
| 特徴量と目的変数の関係を可視化 | データの傾向を見ない |
| 誤分類サンプルの特徴を調査 | 成功例だけ見る |
...
# 発見 1: 性別が最重要特徴
# ↓ ドキュメントに反映
# README に「性別が生存に最も影響する」と記載

# 発見 2: クラス不均衡率 1.67
# ↓ テストに反映
def test_クラス重み付けの効果(self):
    """class_weight='balanced' が不均衡に対応していることを確認"""
    # 不均衡データでの性能を検証
    assert classifier.class_weight == 'balanced'

# 発見 3: 誤分類は主に中間的な特徴量を持つサンプル
# ↓ 改善案
# - 特徴量エンジニアリング: 交互作用項の追加
# - モデルの複雑化: max_depth の調整
```
...
```bash
uv run python script/train_boston.py
...
高度な回帰問題では、特徴量エンジニアリングと標準化の効果を可視化することが重要です。Jupyter Notebook で詳しく分析しましょう！
...
`notebook/` ディレクトリに `boston_exploration.ipynb` を作成しましょう。
...
axes[0].set_xlabel('住宅価格 ($1000s)', fontsize=12)
axes[0].set_ylabel('頻度', fontsize=12)
axes[0].set_title('住宅価格の分布', fontsize=14)
axes[0].axvline(df['PRICE'].mean(), color='red', linestyle='--', lw=2, label=f'平均 ${df["PRICE"].mean():.2f}k')
axes[0].legend()

# 箱ひげ図
axes[1].boxplot(df['PRICE'], vert=True)
axes[1].set_ylabel('住宅価格 ($1000s)', fontsize=12)
axes[1].set_title('住宅価格の箱ひげ図', fontsize=14)
...
print(f"  標準偏差: ${df['PRICE'].std():.2f}k")
...
```python
# セル 4: 各特徴量と価格の散布図
numeric_features = ['RM', 'LSTAT', 'PTRATIO']
fig, axes = plt.subplots(1, 3, figsize=(18, 5))

for idx, feature in enumerate(numeric_features):
    ax = axes[idx]
    ax.scatter(df[feature], df['PRICE'], alpha=0.5, s=30)
    ax.set_xlabel(feature, fontsize=12)
    ax.set_ylabel('PRICE ($1000s)', fontsize=12)
    ax.set_title(f'{feature} と PRICE の関係', fontsize=13)
    ax.grid(True, alpha=0.3)
...
print("相関係数:")
print(df[numeric_features + ['PRICE']].corr()['PRICE'].sort_values(ascending=False))
```
...
# CRIME のダミー変数化
df_encoded = predictor._encode_crime(df)
print(f"エンコーディング後の列: {list(df_encoded.columns)}")

# 訓練データとテストデータに分割
train_df, test_df = train_test_split(df_encoded, test_size=0.2, random_state=42)

# 欠損値補完
train_df = predictor._fill_missing_values(train_df, fit=True)
test_df = predictor._fill_missing_values(test_df, fit=False)

# 特徴量と目的変数の分離
X_train = train_df[['RM', 'LSTAT', 'PTRATIO']]
y_train = train_df[['PRICE']]
X_test = test_df[['RM', 'LSTAT', 'PTRATIO']]
y_test = test_df[['PRICE']]

print(f"\n訓練データ: X={X_train.shape}, y={y_train.shape}")
...
```python
# セル 6: 特徴量エンジニアリング前後の比較
X_train_fe = predictor.feature_engineering(X_train.copy())
print(f"元の特徴量数: {X_train.shape[1]}")
print(f"エンジニアリング後: {X_train_fe.shape[1]}")
print(f"\n追加された特徴量:")
...
print("\n新特徴量と価格の相関:")
for feat in sorted(new_features):
    corr = X_train_fe[feat].corr(y_train['PRICE'])
...
```python
# セル 7: 標準化前後のスケール比較
X_train_fe_copy = X_train_fe.copy()
X_train_scaled = predictor.standardize_features(X_train_fe_copy, fit=True)

fig, axes = plt.subplots(1, 2, figsize=(14, 6))

# 標準化前
axes[0].boxplot([X_train_fe[col] for col in X_train_fe.columns[:5]],
                labels=X_train_fe.columns[:5])
axes[0].set_title('標準化前のスケール', fontsize=14)
axes[0].set_ylabel('値', fontsize=12)
axes[0].tick_params(axis='x', rotation=45)
axes[0].grid(True, alpha=0.3)

# 標準化後
axes[1].boxplot([X_train_scaled[:, i] for i in range(5)],
                labels=X_train_fe.columns[:5])
axes[1].set_title('標準化後のスケール', fontsize=14)
axes[1].set_ylabel('標準化された値', fontsize=12)
...
X_test_fe = predictor.feature_engineering(X_test.copy())
X_test_scaled = predictor.standardize_features(X_test_fe, fit=False)

# 訓練
predictor.train(X_train_scaled, y_train_scaled)

# 予測
y_pred_train_scaled = predictor.predict(X_train_scaled)
y_pred_test_scaled = predictor.predict(X_test_scaled)

# 元のスケールに戻す
y_pred_train = predictor.inverse_transform_prediction(y_pred_train_scaled)
y_pred_test = predictor.inverse_transform_prediction(y_pred_test_scaled)
...
print(f"テストデータ MAE: ${mae_test:.2f}k")
print(f"テストデータ RMSE: ${rmse_test:.2f}k")
```
...
ax1.plot([y_train.min(), y_train.max()],
         [y_train.min(), y_train.max()],
         'r--', lw=2, label='理想的な予測')
ax1.set_xlabel('実測値 ($1000s)', fontsize=12)
ax1.set_ylabel('予測値 ($1000s)', fontsize=12)
ax1.set_title(f'訓練データ (R²={r2_train:.3f})', fontsize=14)
ax1.legend()
ax1.grid(True, alpha=0.3)

# テストデータ
ax2.scatter(y_test, y_pred_test, alpha=0.5, s=30, color='green')
ax2.plot([y_test.min(), y_test.max()],
         [y_test.min(), y_test.max()],
         'r--', lw=2, label='理想的な予測')
ax2.set_xlabel('実測値 ($1000s)', fontsize=12)
ax2.set_ylabel('予測値 ($1000s)', fontsize=12)
ax2.set_title(f'テストデータ (R²={r2_test:.3f})', fontsize=14)
ax2.legend()
ax2.grid(True, alpha=0.3)
...
```python
# セル 10: 残差プロット
residuals_test = y_test.values.flatten() - y_pred_test.flatten()
...
axes[0, 0].axhline(y=0, color='r', linestyle='--', lw=2)
axes[0, 0].set_xlabel('予測値 ($1000s)')
axes[0, 0].set_ylabel('残差 ($1000s)')
...
axes[1, 0].set_title('Q-Q プロット（正規性検証）')
axes[1, 0].grid(True, alpha=0.3)

# 残差の絶対値 vs 予測値
axes[1, 1].scatter(y_pred_test, np.abs(residuals_test), alpha=0.5)
axes[1, 1].set_xlabel('予測値 ($1000s)')
axes[1, 1].set_ylabel('残差の絶対値 ($1000s)')
axes[1, 1].set_title('予測値と誤差の大きさの関係')
axes[1, 1].grid(True, alpha=0.3)
...
print(f"残差の統計:")
print(f"  平均: ${residuals_test.mean():.2f}k")
print(f"  標準偏差: ${residuals_test.std():.2f}k")
print(f"  最大誤差: ${np.abs(residuals_test).max():.2f}k")
```
...
plt.xlabel('係数の値', fontsize=12)
plt.ylabel('特徴量', fontsize=12)
plt.title('線形回帰の係数（標準化後）', fontsize=14)
plt.axvline(x=0, color='black', linestyle='--', lw=1)
...
#### 1️⃣1️⃣ 特徴量エンジニアリングの効果比較

```python
# セル 12: エンジニアリングあり/なしの性能比較
from sklearn.linear_model import LinearRegression

# エンジニアリングなしのモデル
model_no_fe = LinearRegression()
X_train_no_fe = predictor.standardize_features(X_train.copy(), fit=True)
model_no_fe.fit(X_train_no_fe, y_train_scaled)

X_test_no_fe = predictor.standardize_features(X_test.copy(), fit=False)
y_pred_no_fe = model_no_fe.predict(X_test_no_fe)
y_pred_no_fe_original = predictor.inverse_transform_prediction(y_pred_no_fe)

r2_no_fe = r2_score(y_test, y_pred_no_fe_original)

# 比較
comparison = pd.DataFrame({
    '指標': ['R²スコア', '特徴量数'],
    'エンジニアリングなし': [f'{r2_no_fe:.4f}', X_train.shape[1]],
    'エンジニアリングあり': [f'{r2_test:.4f}', X_train_fe.shape[1]]
})

print(comparison.to_string(index=False))
print(f"\n性能向上: {(r2_test - r2_no_fe) / r2_no_fe * 100:.1f}%")
```

#### 💡 高度な回帰問題での Notebook 活用ポイント

| ✅ やること | ❌ 避けること |
|------------|-------------|
| 特徴量エンジニアリングの効果を定量化 | 闇雲に特徴量を増やす |
| 標準化前後のスケールを確認 | 標準化の必要性を無視 |
| 残差の正規性を Q-Q プロットで検証 | 残差の分布を見ない |
| 回帰係数から特徴量の寄与を解釈 | 係数の意味を考えない |

#### 🔄 探索から本番コードへ

```python
# 発見 1: 2乗項と交互作用項で性能が向上
# ↓ テストに反映
def test_特徴量エンジニアリングの効果(self):
    """特徴量エンジニアリングが性能向上に寄与することを確認"""
    # エンジニアリングなし vs ありで R² を比較
    assert r2_with_fe > r2_without_fe

# 発見 2: 標準化により係数の解釈が容易に
# ↓ ドキュメントに反映
# 「標準化された係数は特徴量の相対的重要度を示す」と記載

# 発見 3: LSTAT の2乗項が特に重要
# ↓ 本番コードに反映
# feature_engineering で LSTAT^2 を必ず含める
```
...
FastAPI は統合テストのために `TestClient` を提供しています。これを使うと、実際のサーバーを起動せずに API をテストできます。
...
#### pytest フィクスチャによる共通設定

pytest の `@pytest.fixture` を使って、テストで使用する共通オブジェクトを定義します。

**test/conftest.py を作成**:
...
```
test/test_integration.py::TestAPIIntegration::test_ルートエンドポイントが正しく動作する PASSED
test/test_integration.py::TestAPIIntegration::test_ヘルスチェックが正しく動作する PASSED
test/test_integration.py::TestAPIIntegration::test_Iris予測のエンドツーエンドフロー PASSED
test/test_integration.py::TestAPIIntegration::test_Iris予測で不正なデータを拒否する PASSED
test/test_integration.py::TestAPIIntegration::test_Cinema予測のエンドツーエンドフロー PASSED
test/test_integration.py::TestAPIIntegration::test_Cinema予測でactor範囲外を拒否する PASSED
test/test_integration.py::TestAPIIntegration::test_Survived予測のエンドツーエンドフロー PASSED
test/test_integration.py::TestAPIIntegration::test_Survived予測で不正なsexを拒否する PASSED
test/test_integration.py::TestAPIIntegration::test_Boston予測のエンドツーエンドフロー PASSED
test/test_integration.py::TestAPIIntegration::test_Boston予測で必須フィールド欠落を拒否する PASSED
test/test_integration.py::TestAPIIntegration::test_存在しないエンドポイントで404を返す PASSED
test/test_integration.py::TestAPIIntegration::test_複数のエンドポイントを連続して呼び出せる PASSED
...
- [スッキリわかるPythonによる機械学習入門 第2版 スッキリわかるシリーズ](https://sukkiri.jp/books/sukkiri_ml2)
