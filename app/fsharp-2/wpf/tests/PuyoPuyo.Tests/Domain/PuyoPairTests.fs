...
let ``ぷよペアを作成できる`` () =
    // Arrange & Act
    let pair = PuyoPair.create 2 0 PuyoColor.Red PuyoColor.Green 0

    // Assert
    pair.X |> should equal 2
    pair.Y |> should equal 0
    pair.Puyo1Color |> should equal PuyoColor.Red
    pair.Puyo2Color |> should equal PuyoColor.Green
...
let ``回転状態1のとき2つ目のぷよは右にある`` () =
    // Arrange
    let pair = PuyoPair.create 2 5 PuyoColor.Red PuyoColor.Green 1
...
let ``回転状態2のとき2つ目のぷよは下にある`` () =
    // Arrange
    let pair = PuyoPair.create 2 5 PuyoColor.Red PuyoColor.Green 2
