...
describe('StrategySettings', () => {
  ...
  test('戦略一覧が表示される', async () => {
    // Arrange & Act
    render(<StrategySettings strategyService={mockStrategyService as unknown as StrategyService} />)
  ...
  test('アクティブ戦略が強調表示される', async () => {
    ...
    // Assert
    await waitFor(() => {
      const balancedStrategy = screen.getByText('バランス型').closest('[data-testid="strategy-item"]')
  ...
  test('戦略を選択してアクティブに設定できる', async () => {
    ...
    // Act
    const aggressiveStrategy = screen.getByText('攻撃型').closest('[data-testid="strategy-item"]')!
  ...
  test('カスタム戦略を削除できる', async () => {
    ...
    // Act
    const customStrategyItem = screen.getByText('テストカスタム戦略').closest('[data-testid="strategy-item"]')!
    const deleteButton = customStrategyItem.querySelector('[data-testid="delete-strategy"]')!
  ...
  test('デフォルト戦略は削除できない', async () => {
    ...
    await waitFor(() => {
      const balancedStrategy = screen.getByText('バランス型').closest('[data-testid="strategy-item"]')!
      expect(balancedStrategy.querySelector('[data-testid="delete-strategy"]')).not.toBeInTheDocument()
    })
  })
  ...
  test('ローディング状態が表示される', () => {
    ...
    const loadingService = {
      getAllStrategies: vi.fn(() => new Promise(resolve => setTimeout(resolve, 1000))),
      getActiveStrategy: vi.fn(() => new Promise(resolve => setTimeout(resolve, 1000))),
  ...
  test('エラー状態が表示される', async () => {
    ...
    // Assert
    await waitFor(() => {
      expect(screen.getByText('戦略の読み込みに失敗しました')).toBeInTheDocument()
    })
  })
})