...
let ``ボードにぷよを配置できる`` () =
    ...
    // Act
    let 新しい盤面 = 盤面.セル設定 _盤面 2<列> 10<行> (埋まっている 赤)
...
let ``ボードにぷよを配置しても元のボードは変更されない`` () =
    ...
    // Assert
    盤面.セル取得 _盤面 2<列> 10<行> |> should equal 空
    盤面.セル取得 新しい盤面 2<列> 10<行> |> should equal (埋まっている 赤)
...
let ``指定位置のぷよを消去できる`` () =
    ...
    // Act
    let 消去後の盤面 = 盤面.ぷよ消去 _盤面 [ (2<列>, 10<行>); (2<列>, 11<行>) ]
...
let ``ぷよ消去後も元の盤面は変更されない`` () =
    ...
    let _盤面 = _盤面 |> (fun 盤 -> 盤面.セル設定 盤 2<列> 10<行> (埋まっている 赤))

    // Act
    let 消去後の盤面 = 盤面.ぷよ消去 _盤面 [ (2<列>, 10<行>) ]

    // Assert
    盤面.セル取得 _盤面 2<列> 10<行> |> should equal (埋まっている 赤)
    盤面.セル取得 消去後の盤面 2<列> 10<行> |> should equal 空
...
let ``消去と重力を繰り返し適用で連鎖が自動的に処理される`` () =
    ...
    盤面.セル取得 連鎖処理後の盤面 1<列> 10<行> |> should equal 空
    盤面.セル取得 連鎖処理後の盤面 2<列> 10<行> |> should equal 空
    盤面.セル取得 連鎖処理後の盤面 1<列> 11<行> |> should equal 空
    盤面.セル取得 連鎖処理後の盤面 2<列> 11<行> |> should equal 空
    盤面.セル取得 連鎖処理後の盤面 3<列> 10<行> |> should equal 空
    盤面.セル取得 連鎖処理後の盤面 2<列> 7<行> |> should equal 空
    盤面.セル取得 連鎖処理後の盤面 2<列> 8<行> |> should equal 空
    盤面.セル取得 連鎖処理後の盤面 2<列> 9<行> |> should equal 空
