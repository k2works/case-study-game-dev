...
let ``ぷよペアを作成できる`` () =
    // Arrange & Act
    let ぷよペア = ぷよペア.作成 2<列> 0<行> 赤 緑 0<回転>
    ...
    ぷよペア.回転状態 |> should equal 0<回転>

[<Fact>]
let ``回転状態0のとき2つ目のぷよは上にある`` () =
    // Arrange
    let ペア = ぷよペア.作成 2<列> 5<行> 赤 緑 0<回転>
...
let ``回転状態1のとき2つ目のぷよは右にある`` () =
    // Arrange
    let ペア = ぷよペア.作成 2<列> 5<行> 赤 緑 1<回転>
...
let ``回転状態2のとき2つ目のぷよは下にある`` () =
    // Arrange
    let ペア = ぷよペア.作成 2<列> 5<行> 赤 緑 2<回転>
...
let ``回転状態3のとき2つ目のぷよは左にある`` () =
    // Arrange
    let ペア = ぷよペア.作成 2<列> 5<行> 赤 緑 3<回転>
...
let ``回転状態3から時計回りに回転すると0に戻る`` () =
    ...
    // Assert
    回転後.回転状態 |> should equal 0<回転>

[<Fact>]
let ``回転すると2つ目のぷよの位置が変わる`` () =
    // Arrange
    let ペア = ぷよペア.作成 3<列> 5<行> 赤 緑 0<回転> // 回転状態0（上）
