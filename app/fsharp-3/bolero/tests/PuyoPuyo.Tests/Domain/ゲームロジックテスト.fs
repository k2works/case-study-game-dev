...
let ``空いている位置では回転できる`` () =
    // Arrange
    let 盤面 = 盤面.作成 6<列> 13<行>
    let ペア = ぷよペア.作成 2<列> 5<行> 赤 緑 0<回転> // 回転状態0（上）
    ...
    // Assert
    match 結果 with
    | Some 回転後のペア -> 回転後のペア.回転状態 |> should equal 1<回転>
...
let ``壁に当たる位置では回転できない`` () =
    // Arrange
    let 盤面 = 盤面.作成 6<列> 13<行>
    let ペア = ぷよペア.作成 5<列> 5<行> 赤 緑 0<回転> // 回転状態0（上）
...
let ``右端で右向き回転のとき左にずらして回転できる`` () =
    // Arrange
    let 盤面 = 盤面.作成 6<列> 13<行>
    let ペア = ぷよペア.作成 5<列> 5<行> 赤 緑 0<回転> // 回転状態0（上）、列5（右端）
    ...
    match 結果 with
    | Some 回転後のペア ->
        回転後のペア.回転状態 |> should equal 1<回転> // 回転状態1（右）
...
let ``左端で左向き回転のとき右にずらして回転できる`` () =
    // Arrange
    let 盤面 = 盤面.作成 6<列> 13<行>
    let ペア = ぷよペア.作成 0<列> 5<行> 赤 緑 2<回転> // 回転状態2（下）、列0（左端）
    ...
    match 結果 with
    | Some 回転後のペア ->
        回転後のペア.回転状態 |> should equal 3<回転> // 回転状態3（左）
...
let ``下端では下に移動できない`` () =
    // Arrange
    let 盤面 = 盤面.作成 6<列> 13<行>
    let ペア = ぷよペア.作成 3<列> 12<行> 赤 緑 0<回転> // 回転状態0（上）、軸ぷよが y=12（下端）
...
let ``下にぷよがある場合は移動できない`` () =
    ...
    let ペア = ぷよペア.作成 3<列> 5<行> 赤 緑 0<回転> // 軸ぷよ (3,5)、2つ目 (3,4)
...
let ``新しいぷよを配置できない場合、ゲームオーバーになる`` () =
    ...
    let 新しいぷよペア = ぷよペア.作成 2<列> 1<行> 青 緑 0<回転>

    // Act - ゲームオーバー判定
    let ゲームオーバーか = ゲームロジック.ゲームオーバー判定 新しい盤面 新しいぷよペア
...
let ``新しいぷよを配置できる場合、ゲームオーバーにならない`` () =
    ...
    // Act - ゲームオーバー判定
    let ゲームオーバーか = ゲームロジック.ゲームオーバー判定 盤面 新しいぷよペア
...
let ``ぷよペアの回転位置も考慮してゲームオーバー判定する`` () =
    ...
    let 新しい盤面 = 現在の盤面 |> (fun 盤 -> 盤面.セル設定 盤 2<列> 1<行> (埋まっている 赤)) // 回転後の位置にぷよがある
...
let ``ぷよが盤面の上部ギリギリでもゲームオーバーにならない`` () =
    ...
    let 新しい盤面 = 現在の盤面 |> (fun 盤 -> 盤面.セル設定 盤 2<列> 11<行> (埋まっている 赤))
