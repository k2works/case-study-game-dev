...
let ``回転メッセージでぷよペアが回転する`` () =
    // Arrange
    let 盤面 = 盤面.作成 6<列> 13<行>
    let ぷよペア = ぷよペア.作成 2<列> 5<行> 赤 緑 0 // 回転状態0
...
let ``右端の回転メッセージで壁キックが発動する`` () =
    // Arrange
    let 盤面 = 盤面.作成 6<列> 13<行>
    let ぷよペア = ぷよペア.作成 5<列> 5<行> 赤 緑 0 // 右端、回転状態0
...
let ``着地したぷよはボードに固定され次のぷよが生成される`` () =
    // Arrange
    let 盤面 = 盤面.作成 6<列> 13<行>
    let ペア = ぷよペア.作成 3<列> 12<行> 赤 緑 0 // 下端
    let 次のぷよ = ぷよペア.作成 2<列> 1<行> 青 黄 0
    ...
    // Assert
    // 着地したぷよがボードに固定されている
    PuyoPuyo.Domain.盤面.セル取得 新しいモデル.盤面 3<列> 12<行> |> should equal (埋まっている 赤)
    PuyoPuyo.Domain.盤面.セル取得 新しいモデル.盤面 3<列> 11<行> |> should equal (埋まっている 緑)
...
let ``着地後に4つ以上つながったぷよが消去される`` () =
    ...
    let ペア = ぷよペア.作成 2<列> 9<行> 赤 赤 2 // 回転状態2: ぷよ1が(2,9)、ぷよ2が(2,10)
    let 次のぷよ = ぷよペア.作成 2<列> 1<行> 青 黄 0
    ...
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 12<行> |> should equal 空
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 11<行> |> should equal 空
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 10<行> |> should equal 空
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 9<行> |> should equal 空
...
let ``消去後に重力が適用されて浮いたぷよが落下する`` () =
    ...
    let 配置後の盤面 =
        初期盤面
        |> (fun 盤 -> PuyoPuyo.Domain.盤面.セル設定 盤 2<列> 7<行> (埋まっている 青))
        |> (fun 盤 -> PuyoPuyo.Domain.盤面.セル設定 盤 2<列> 11<行> (埋まっている 赤))
        |> (fun 盤 -> PuyoPuyo.Domain.盤面.セル設定 盤 2<列> 12<行> (埋まっている 赤))

    // 赤いぷよペアを落とす (2,9)と(2,10)に配置
    let ペア = ぷよペア.作成 2<列> 9<行> 赤 赤 2
    let 次のぷよ = ぷよペア.作成 2<列> 1<行> 青 黄 0
    ...
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 12<行> |> should equal (埋まっている 青)
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 11<行> |> should equal 空
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 10<行> |> should equal 空
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 9<行> |> should equal 空
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 7<行> |> should equal 空
...
let ``消去がない場合も重力が適用される`` () =
    ...
    // 列2の行12に赤を配置
    let 配置後の盤面 = 初期盤面 |> (fun 盤 -> PuyoPuyo.Domain.盤面.セル設定 盤 2<列> 12<行> (埋まっている 赤))

    // 縦向きのぷよペア（回転状態0: ぷよ2が上）を落とす
    // ペアが着地したとき、下のぷよ(2,11)は固定されるが、上のぷよ(2,10)は浮いている
    // 重力適用後、上のぷよも(2,11)に詰められるはず
    let ペア = ぷよペア.作成 2<列> 11<行> 青 緑 0 // ぷよ1:(2,11), ぷよ2:(2,10)
    let 次のぷよ = ぷよペア.作成 2<列> 1<行> 青 黄 0
    ...
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 12<行> |> should equal (埋まっている 赤)
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 11<行> |> should equal (埋まっている 青)
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 10<行> |> should equal (埋まっている 緑)
    PuyoPuyo.Domain.盤面.セル取得 モデル2.盤面 2<列> 9<行> |> should equal 空
