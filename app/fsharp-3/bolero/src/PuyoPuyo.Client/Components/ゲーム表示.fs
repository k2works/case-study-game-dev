...
module ゲーム画面 =
    ...
    /// 盤面内かチェック
    let private 盤面内か (盤面: 盤面) (列: int<列>) (行: int<行>) : bool =
        行 >= 0<行> && 行 < 盤面.行数 && 列 >= 0<列> && 列 < 盤面.列数

    /// ぷよをボードコピーに配置
    let private ぷよを配置 (ボードのコピー: セル[][]) (盤面: 盤面) (列: int<列>) (行: int<行>) (色: ぷよの色) : unit =
        if 盤面内か 盤面 列 行 then
            ボードのコピー.[int 行].[int 列] <- 埋まっている 色
    ...
    let private ボードを描画 (盤面: 盤面) (現在のぷよペア: ぷよペア option) =
        ...
        | Some ピース ->
            ...
            let (列1, 行1) = 位置1
            let (列2, 行2) = 位置2
            ぷよを配置 ボードのコピー 盤面 列1 行1 ピース.ぷよ1の色
            ぷよを配置 ボードのコピー 盤面 列2 行2 ピース.ぷよ2の色
        | None -> ()
