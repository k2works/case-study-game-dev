...
module ぷよペア =
    /// ぷよペアを作成
    let 作成 (列: int<列>) (行: int<行>) (色1: ぷよの色) (色2: ぷよの色) (回転: int) : ぷよペア =
        { X座標 = 列
          Y座標 = 行
          ぷよ1の色 = 色1
          ぷよ2の色 = 色2
    ...
    let 位置取得 (ぷよペア: ぷよペア) : (int<列> * int<行>) * (int<列> * int<行>) =
        ...
        let 位置2 =
            ...
            | 0 -> (ぷよペア.X座標, ぷよペア.Y座標 - 1<行>) // 上
            | 1 -> (ぷよペア.X座標 + 1<列>, ぷよペア.Y座標) // 右
            | 2 -> (ぷよペア.X座標, ぷよペア.Y座標 + 1<行>) // 下
            | 3 -> (ぷよペア.X座標 - 1<列>, ぷよペア.Y座標) // 左
            | _ -> (ぷよペア.X座標, ぷよペア.Y座標 - 1<行>) // デフォルトは上

        (位置1, 位置2)

    /// ランダムなぷよペアを生成
    let ランダム作成 (列: int<列>) (行: int<行>) (回転: int) : ぷよペア =
